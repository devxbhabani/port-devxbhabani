...
const ManifestoWord = ({ children, progress, range }) => {
  ...
  return (
    <motion.span style={{ opacity }} className="mr-[1vw] mt-[0.5vw] inline-block">
      {children}
    </motion.span>
  );
};
...
const About = () => {
  ...
  return (
    <section id="about" ref={containerRef} className="relative w-full bg-white h-[250vh]">
      <div className="sticky top-0 h-screen w-full flex flex-col justify-end pb-[10vh] md:pb-[15vh] px-8 md:px-[10vw] overflow-hidden">
        <div className="w-full max-w-[1400px]">
          <p className="text-zinc-400 uppercase tracking-widest text-xs md:text-sm mb-4 md:mb-6 font-bold">Manifesto</p>
          <h2 className="text-[7vw] md:text-[2.75vw] leading-[1.1] tracking-tight font-black text-black flex flex-wrap">
            {words.map((word, i) => {
              const start = i / words.length;
              const end = start + (1 / words.length);
              return (
                <ManifestoWord key={i} progress={scrollYProgress} range={[start, end]}>
                  {word}
                </ManifestoWord>
              );
            })}
